package metrics

import (
	"context"
	"github.com/payloadops/lanyard/app/config"
	"github.com/stretchr/testify/assert"
	"testing"
)

func TestNewMeter_WithCACert(t *testing.T) {
	// Set up the environment variables
	cfg := &config.Config{
		OpenTelemetry: config.OpenTelemetryConfig{
			ProviderEndpoint: "localhost:4317",
			CACert: `-----BEGIN CERTIFICATE-----

9qKpcBScxVoYU/18dhsZ
-----END CERTIFICATE-----`,
		},
	}

	mp, err := NewMeter(context.Background(), cfg)
	assert.NoError(t, err)
	assert.NotNil(t, mp)
}

func TestNewMeter_WithoutCACert(t *testing.T) {
	// Set up the environment variables
	cfg := &config.Config{
		OpenTelemetry: config.OpenTelemetryConfig{
			ProviderEndpoint: "localhost:4317",
			CACert:           "",
		},
	}

	mp, err := NewMeter(context.Background(), cfg)
	assert.NoError(t, err)
	assert.NotNil(t, mp)
}

func TestNewMeter_InvalidCACert(t *testing.T) {
	// Set up the environment variables
	cfg := &config.Config{
		OpenTelemetry: config.OpenTelemetryConfig{
			ProviderEndpoint: "localhost:4317",
			CACert:           "invalid-cert",
		},
	}

	mp, err := NewMeter(context.Background(), cfg)
	assert.Error(t, err)
	assert.Nil(t, mp)
}
