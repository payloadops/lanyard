...
export class EcsStack extends cdk.Stack {
  constructor(scope: Construct, id: string, vpcStack: VpcStack, stage: string, props?: cdk.StackProps) {
    ...
    const ecrRepository = Stages.STAGING === stage && Regions.US_WEST_2 === region ? new ecr.Repository(scope, disambiguator('Repository', stage, region), 
    {
      repositoryName: 'app',
      removalPolicy: cdk.RemovalPolicy.RETAIN // Adjust the policy based on your needs
    }) : ecr.Repository.fromRepositoryArn(scope, disambiguator('Repository', stage, region), `arn:aws:ecr:${Regions.US_WEST_2}:${Accounts.STAGING}:repository/app`)
